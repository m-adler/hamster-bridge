...
class RedmineHamsterListener(HamsterListener):
    ...
    config_values = [
        ConfigValue(
            key="server_url",
            setup_func=lambda: input(
                'Root URL to the Redmine server [f.e. "http://redmine.example.org/"]\n'
            ),
            sensitive=False,
        ),
        ConfigValue(
        ...
        ConfigValue(
            key="auto_start",
            setup_func=lambda: input(
                "Automatically start the issue when you start the task in hamster? [y/n]\n"
            ),
            sensitive=False,
        ),
        ConfigValue(
    ...
    def __filter_issue_statuses(self):
        ...
        def find_default(element):
            ...
            """
            return hasattr(element, "is_default") and getattr(
        ...
        try:
            self.__issue_status_default = [
                item for item in issue_statuses if find_default(item)
            ][0]
        except IndexError:
            logger.exception("Unable to find a single default issue status!")

        try:
            self.__issue_status_in_work = [
                item for item in issue_statuses if find_in_work(item)
            ][0]
        except IndexError:
    ...
    def __get_first_activity_id(self):
        ...
        :rtype: int
        """
        return [key for key, value in list(self.__activities.items()) if value[1]][0]
    ...
    def prepare(self):
        ...
        if verify_ssl.lower() in ("y", "true"):
            ...
            logger.info(
                "Enabling SSL/TLS certificate verification (custom CA " "path) '%s'",
                verify_ssl,
            )
            requests_dict["verify"] = verify_ssl
        else:
            logger.error(
                "verify_ssl = '%s' is not a valid CA cert path nor a "
                "valid option. Falling back to enabling SSL/TLS verification "
                "with default CA path",
                verify_ssl,
            )
            requests_dict["verify"] = True
        ...
        time_entry_activities = self.redmine.enumeration.filter(
        ...
        try:
            ...
            logger.exception(
                "Unable to communicate with redmine server. See error in the following output:"
            )
    ...
    def on_fact_started(self, fact):
        ...
        if auto_start == "y":
            ...
            if not issue:
                logger.error(
                    "Unable to query issue for starting of hamster fact %s",
                    fact.original_activity,
                )
                return
    ...
    def on_fact_stopped(self, fact):
        ...
        if not issue:
            logger.error(
                "Unable to query issue for stopping of hamster fact %s",
                fact.original_activity,
            )
            return
