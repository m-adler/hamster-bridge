...
class JiraHamsterListener(HamsterListener):
    ...
    config_values = [
        ConfigValue(
            ...
            setup_func=lambda: input(
                'Root url of your jira server [f.e. "http://jira.example.org"]: '
            ),
            sensitive=False,
        ),
        ConfigValue(
        ...
        ConfigValue(
            key="auto_start",
            setup_func=lambda: input(
                "Automatically start the issue when "
    ...
    def prepare(self):
        ...
        self.jira = JIRA(server_url, options=options, basic_auth=(username, password))
        ...
        for field in fields:
            for possible_issue in self.issue_from_title.findall(field):
                ...
                try:
                    ...
                    logger.debug(
                        'Found existing issue "%s" in "%s"', possible_issue, field
                    )
                    return possible_issue
                except JIRAError as e:
                    if e.text == "Issue Does Not Exist":
                        logger.warning(
                            'Tried issue "%s", but does not exist. ', possible_issue
                        )
                    else:
        ...
        try:
            ...
            for transition in transitions:
                if transition["name"] == transition_name:
                    ...
                    logger.info(
                        'Marked issue "%s" as "%s"', issue_name, transition_name
                    )
        ...
        if issue_name:
            try:
                ...
                worklog = self.jira.add_worklog(
                    issue_name, time_spent, started=tstart, comment=fact.description
                )
                logger.info(
                    "Logged work: %s - %s to %s (created %r)",
                    fact.start_time,
                    time_spent,
                    issue_name,
                    worklog,
